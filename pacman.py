...
def distance(p0, p1):  # euclidian 
    return math.sqrt((p0[0] - p1[0]) ** 2 + (p0[1] - p1[1]) ** 2)


class PacmanGame:
    def __init__(self, field_shape, nmonsters, ndiamonds, nwalls,
                 monster_vision_range=1, max_moves=1000,
    ...
    def render(self, pixels_per_square=50, score_height=100):
        ...
        if self.game_window is None:
            ...
            self.player_img = pyglet.resource.image("ladyP.png")
            self.monster_img = pyglet.resource.image("inky.png")
            self.diamond_img = pyglet.resource.image("diamond.png")
            self.wall_img = pyglet.resource.image("block0.PNG")
